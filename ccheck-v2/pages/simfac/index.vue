...
watch([subject], () => {
  updateTableData(subject.value);
});


const resetFilter = () => {
  // รีเซ็ต header, index, และ row ให้เป็นค่าเริ่มต้นที่ดึงมาจาก API
  header.value = [...initialHeader.value];  // คืนค่า header เดิม
  index.value = [...initialIndex.value];    // คืนค่า index เดิม
  row.value = [...initialRow.value];        // คืนค่า row เดิมพร้อมค่า similarity
};

...
const getCurriculums = async (
  ...
  pending.value = true;
  let apiEndpoint = `/api/curriculums?limit=${limit}&offset=${offset}`;
  if (name) {
...
const deleteCurriculum = async (pk: number) => {
  await $fetch(`/api/curriculums/${pk}`, {
    method: "DELETE",
    onResponse: ({ response }) => {
      if (!response.ok) {
        toast.add({
      ...
      toast.add({
        ...
        description: "ข้อมูลหลักสูตรถูกลบเรียบร้อยแล้ว",
        color: "green",
        icon: "i-heroicons-information-circle",
      });
      getCurriculums(
...
<template>
  <UContainer class="my-4">
    <UCard>
      ...
      <h1 class="text-3xl text-center my-8">ข้อมูลหลักสูตร</h1>
      <div class="flex md:flex-row flex-col gap-2 justify-between my-4">
        ...
        <UButton icon="i-heroicons-plus-20-solid" to="/curriculums/create">
          เพิ่มหลักสูตร
        </UButton>
      </div>
      <UTable
      ...
      >
        <template #actions-data="{ row }">
          <div class="flex md:flex-row flex-col gap-4">
            <UButton
              :to="`/curriculums/${row.pk}/subjects`"
              icon="i-heroicons-book-open-20-solid"
              square
            >
            </UButton>
            <UButton
              :to="`/curriculums/${row.pk}/edit`"
              color="amber"

...
const {
  ...
  error,
  refresh,
} = await useFetch("/api/degrees");

const q = ref("");
const page = ref(1);
const perPage = ref(5);
const searchDegrees = computed(() => {
  if (!degrees.value) {
    return [];
  }
  ...
  return degrees.value.filter((degree) => {
    return degree.name.toLowerCase().includes(q.value.toLowerCase());
  });
});

const rows = computed(() => {
  return searchDegrees.value.slice(
  ...
  await $fetch(`/api/degrees/${id}`, {
    ...
    onResponse: ({ response }) => {
      ...
      toast.add({
        ...
        description: "ข้อมูลระดับการศึกษาถูกลบเรียบร้อยแล้ว",
        color: "green",
        icon: "i-heroicons-information-circle",
      });
      refresh();
...
<template>
  <UContainer class="my-4">
    <UCard>
      ...
      <h1 class="text-3xl text-center my-8">ข้อมูลระดับการศึกษา</h1>
      <div class="flex md:flex-row flex-col gap-2 justify-between my-4">
        <div class="flex md:flex-row flex-col gap-2">
          <UInput v-model="q" placeholder="ค้นหาชื่อระดับหลักสูตร..." />
          <UButton
            :loading="pending"
            color="gray"
        ...
        <UButton icon="i-heroicons-plus-20-solid" to="/degrees/create">
          เพิ่มระดับการศึกษา
        </UButton>
      </div>
    ...
    <UModal v-model="isDeleteModal">
      <div class="p-4">
        ...
        <div class="text-center text-xl my-8">{{ selectedDegree?.name }}</div>
        <div class="flex justify-center gap-4 my-4">
          <UButton color="red" @click="closeModal()">ยกเลิก</UButton>
          <UButton color="green" @click="deleteDegree(selectedDegree!.id)">
            ยืนยัน
          </UButton>
        </div>
      </div>
